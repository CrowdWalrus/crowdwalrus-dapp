    ...
    mutationFn: async ({ formData, options }) => {
      ...
      try {
        ...
        try {
          ...
          const costEstimate = await calculateStorageCost(
            suiClient,
            network,
            formData,
            storageEpochs,
          ...
          console.log(
            "Total WAL needed:",
            costEstimate.totalCostWal.toFixed(6),
            "WAL",
          );
          console.log(
            "  - Storage cost:",
            costEstimate.storageCostWal.toFixed(6),
            "WAL",
          );
          console.log(
            "  - Upload cost:",
            costEstimate.uploadCostWal.toFixed(6),
            "WAL",
          );
