...
export async function attachPriceOracleQuote({
  ...
  if (!priceUpdateData.length) {
    throw new Error("Hermes returned no price update data for the requested feed.");
  }
  ...
  return {
    priceInfoObject: transaction.object(priceInfoObjectIds[0]),
    quotedUsdMicro,
    publishTimeMs,
    feedId,
...
function extractParsedPrice(
  ...
  const priceEntry = update.parsed?.find((entry) => entry.id === feedId);
...
export function applySlippageTolerance(
  ...
  if (toleranceBps < 0 || toleranceBps > 10_000) {
    throw new Error("Slippage tolerance must be between 0 and 10,000 bps (0%-100%).");
  }
